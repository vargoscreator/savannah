...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function showAuthPopup(mode) {
    ...
    const target = document.querySelector(
      `.authPopup__content[data-authPopup="${mode}"]`
    );
  ...
  document
    .querySelectorAll(".header-login-btn")
    .forEach((btn) =>
      btn.addEventListener("click", () => showAuthPopup("login"))
    );
  document
    .querySelectorAll(".header-register-btn")
    .forEach((btn) =>
      btn.addEventListener("click", () => showAuthPopup("register"))
    );
  document
    .querySelectorAll(".login-btn, .register-btn, .forgot-btn")
    .forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        if (btn.classList.contains("forgot-btn"))
          showAuthPopup("password-recovery");
      });
    });
  document
    .querySelectorAll(".authPopup__close")
    .forEach((btn) => btn.addEventListener("click", () => removeAllActive()));
  ...
  document.querySelectorAll(".authPopup__form").forEach((form) => {
    ...
    form.addEventListener("submit", (e) => {
      ...
      requiredInputs.forEach((input) => {
        const block =
          input.closest(".authPopup__form-block") ||
          input.closest(".authPopup__form-agreed");
        if (
      ...
      if (form.closest('[data-authPopup="password-recovery-code"]')) {
        const codeInputs = form.querySelectorAll(
          '.authPopup__code-input input[type="tel"]'
        );
        ...
        if (enteredCode === "123456") {
          ...
          codeInputs.forEach((i) =>
            i.closest(".authPopup__code-input")?.classList.remove("error")
          );
        } else {
          codeInputs.forEach((i) =>
            i.closest(".authPopup__code-input")?.classList.add("error")
          );
        }
        return;
      }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (editForm) {
    const requiredInputs = editForm.querySelectorAll(
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (btn) {
      ...
      if (!isInsideActivePopup) {
        ...
        const popup = document.querySelector(
          `.popupProduct[data-popupProduct="${popupName}"]`
        );
        if (popup) {
  ...
  const headerCloseBtn = document.querySelector(".inner-close");
