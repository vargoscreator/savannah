...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (window.innerWidth <= 768) {
      ...
      on: {
        slideChangeTransitionStart: function () {
          this.slides.forEach((slide) => {
            let inner = slide.querySelector(
              ".characters__slider--mob .characters__slide-inner"
            );
            if (inner)
              gsap.set(inner.children, {
                clearProps: "all",
              });
          });

          let activeSlide = this.slides[this.activeIndex];
          let inner = activeSlide.querySelector(
            ".characters__slider--mob .characters__slide-inner"
          );
          if (inner) {
    ...
    window.addEventListener("load", () => {
      let firstInner = document.querySelector(
        ".characters__slider--mob .characters__slide.swiper-slide-active .characters__slide-inner"
      );
      if (firstInner) {
    ...
    ScrollTrigger.create({
      ...
      onEnter: () => {
        const activeSlide =
          charactersSlider.slides[charactersSlider.activeIndex];
        animateSlide(activeSlide);
      },
    });
  } else {
    const charactersSlides = gsap.utils.toArray(
      ".characters__slider--desk .characters__slide"
    );
    if (charactersSlides.length > 0) {
      ...
      function goToSlide(index) {
        ...
        currentIndex = Math.max(
          0,
          Math.min(charactersSlides.length - 1, index)
        );
  ...
  const enjoyImages = document.querySelectorAll(
    ".enjoy__icon, .enjoy__title span img:not(:last-child)"
  );
  ...
  if (enjoyImages.length > 0) {
    ...
    const animators = Array.from(enjoyImages).map((img, i) => {
      return {
        ...
        rotation: gsap.quickTo(img, "rotation", {
          duration: 0.6,
          ease: "power2.out",
        }),
      };
  ...
  const championImages = document.querySelectorAll(
    ".champion__title img:first-of-type"
  );
  document.addEventListener("mousemove", (e) => {
    const x = e.clientX;
    const y = e.clientY;
  ...
  tlLevelup.fromTo(
    ".levelup__title span",
    { opacity: 0, y: 40, rotateX: -90 },
    { opacity: 1, y: 0, rotateX: 0, transformPerspective: 1000, stagger: 0.075 }
  );
  ...
  championTitleTl
    .from(
      ".champion__title span:nth-of-type(1)",
      {
        x: -50,
        opacity: 0,
        duration: 0.8,
    ...
    .from(
      ".champion__title span:nth-of-type(2)",
      {
        x: 50,
        opacity: 0,
        duration: 0.8,
    ...
    .from(
      ".champion__title span:nth-of-type(3)",
      {
        x: -50,
        opacity: 0,
        duration: 0.8,
  ...
  activityTl
    .from(".activity__box:first-of-type", {
      x: -100,
      opacity: 0,
      duration: 0.6,
      ease: "power3.out",
  ...
  activityTl
    .from(".activity__middle .activity__name", {
      y: 50,
      opacity: 0,
      duration: 0.5,
      ease: "power3.out",
